...
(() => {
  ...
  // src/components/positioning-slider.ts
  ...
  function initSlider(slider) {
    const getSlides = gsapWithCSS.utils.toArray('[cs-el="slide"]');
    const slidesLength = getSlides.length;
    const slides = getSlides;
    if (slidesLength === 0)
      return;
    slides.forEach((slide) => {
      const radioWraps = document.querySelectorAll('[cs-el="radios-wrap"]');
      radioWraps.forEach((el) => {
        const questionNumber = el.getAttribute("qi");
        const questionArchetype = el.getAttribute("archetype");
        const radios = el.querySelectorAll(".w-radio");
      ...
      const formRadioBtns = gsapWithCSS.utils.toArray(".w-radio", slide);
    ...
    function slideAction(dir, index) {
      ...
      function gsapSlideIn(i) {
        ...
        const radioButtons = gsapWithCSS.utils.toArray(".w-radio", slides[i]);
        gsapWithCSS.fromTo(
          radioButtons,
          { opacity: 0 },
      ...
      function gsapSlideOut(i) {
        ...
        const radioButtons = gsapWithCSS.utils.toArray(".w-radio", slides[i]);
        gsapWithCSS.set(radioButtons, { opacity: 0 });
    ...
    function setProgressBar(totalSlides, currentSlide) {
      const progressBarElement = document.querySelector('[cs-el="slider-progress-bar"]');
      const progressIntElement = document.querySelector('[cs-el="progress-number"]');
      if (progressBarElement) {
        const widthPercentage = currentSlide / totalSlides * 100;
        gsapWithCSS.to(progressBarElement, {
          duration: 0.5,
    ...
    function infoButtonaddListeners() {
      ...
      if (infoButton) {
        ...
        let isOpen = false;
        if (infoBlock) {
          const tl_openInfo = gsapWithCSS.timeline({ paused: true });
          ...
          const closeButtons = document.querySelectorAll('[cs-el="closeInfo"]');
...
gsap/CSSPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=brandpositioning.js.map
